...
const ForgotPassword = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    // আপাতত আমরা শুধু ফ্রন্টএন্ড ডিজাইন করছি, পরে ব্যাকএন্ড কানেক্ট করব
    try {
      // ব্যাকএন্ড কল হবে এখানে (await axios.post...)
      
      // ডামি সাকসেস মেসেজ (টেস্ট করার জন্য)
      setTimeout(() => {
  ...
  return (
    <div className="min-h-[calc(100vh-80px)] flex items-center justify-center bg-[#f8faff] relative overflow-hidden px-4">
      ...
      <div className="relative z-10 w-full max-w-[420px] bg-white/60 backdrop-blur-xl border border-white/80 shadow-[0_8px_40px_rgba(0,0,0,0.08)] rounded-3xl p-8 sm:p-10">
        ...
        {/* Success Message */}
        {message && (
          <div className="bg-green-50 text-green-700 border border-green-200 px-4 py-3 rounded-xl mb-6 text-sm text-center font-medium animate-fadeIn">
            ✅ {message}
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full bg-[#5e17eb] hover:bg-[#4a11b8] text-white font-bold py-3.5 rounded-xl shadow-lg shadow-purple-200 transform transition hover:-translate-y-1 disabled:opacity-70 flex items-center justify-center gap-2"
          >
            {loading ? (
              <>
